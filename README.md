...
3. Run `make`, which invokes Component to download any required dependencies and package them together with the local otp.js code (a copy of the packaged code is put in `build`).

4. Update the default configuration in `config.js` to point to your OTP backend.

5. Deploy the client to the web by copying the `build/` directory to your web server.
