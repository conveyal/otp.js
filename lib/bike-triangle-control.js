...
var BikeTriangleControl = Backbone.View.extend({
  ...
  render: function () {
    ...
    // Ensure that values are up to date according to any plan request query
    // parameters.
    if (this.options.model.attributes) {
      this.setValues(
        +this.options.model.attributes.triangleTimeFactor,
        +this.options.model.attributes.triangleSlopeFactor,
        +this.options.model.attributes.triangleSafetyFactor
      )
    }
    this.rendered = true
  },
  ...
  setValues: function (quick, flat, safe) {
    if (
      ...
      typeof safe !== 'number' ||
      flat + safe + quick !== 1 ||
      flat < 0 || quick < 0 || safe < 0
    ) {
      console.warn('triangle factors are invalid', quick, flat, safe)
      quick = 0.333
      flat = 0.333
      safe = 0.334
    }
