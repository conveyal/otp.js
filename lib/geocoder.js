 ...
 var EsriGeocoder = {
    ...
    reverse: function(place, error, success) {
 	...
 	lookup : function(query, callback, select) {
        ...
        $.ajax({
            url: window.OTP_config.esriApi + 'suggest?text=' + encodeURIComponent(query) + '&outFields=City,Region&f=pjson&location=',
            type: 'GET',
            dataType : 'jsonp',
...
var SimplecoderGeocoder = {
	...
	reverse: function(place, error, success) {
		$.ajax({
	        url: window.OTP_config.simplecoderApi + '/r/' + encodeURIComponent(place),
	        type: 'GET',
	        error: function() {
	           error();
	        },
