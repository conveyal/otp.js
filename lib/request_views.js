...
var OtpRequestFormView = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    this.model.on('change', function(data) {
      ...
      if (_.has(data.changed, 'fromPlace') && data.attributes.fromPlace &&
        view.selectFrom) {
        view.updateReverseGeocoder('From', data.attributes.fromPlace,
          view.selectTo);

      }

      if (_.has(data.changed, 'toPlace') && data.attributes.toPlace &&
        view.selectTo) {
        ...
        results: function(res, page) {
          ...
          for (var itemPos in res.candidates.slice(0, 10)) {
            ...
            var item = {
              ...
              id: res.candidates[itemPos].location.y + ',' + res.candidates[
                itemPos].location.x
            };
