...
var PlanRequest = Backbone.Model.extend({
  ...
  request: function () {
    if (!this.attributes.fromPlace) {
      ...
      var m = this
      // Replace default routerId alternative routerId found.
      var planRequestUrl = this.attributes.routerId
        ? this.urlRoot.replace(
            'otp/routers/default',
            'otp/routers/' + this.attributes.routerId
          )
        : this.urlRoot
      $.ajax(planRequestUrl, {
        dataType: 'json',
