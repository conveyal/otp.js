...
var PlanRequest = Backbone.Model.extend({
  ...
  request: function () {
    if (!this.attributes.fromPlace) {
      ...
      var m = this
      if (this.attributes.routerId) {
        // If router ID query param is provided, replace router ID in URL root
        // with the specified value.
        var urlParts = this.urlRoot.split('/')
        urlParts[urlParts.length - 2] = this.attributes.routerId
        this.urlRoot = urlParts.join('/')
      }
      $.ajax(this.urlRoot, {
        dataType: 'json',
  ...
  fromQueryString: function (queryString) {
    ...
    // // Set router ID if included in the query string.
    // if (params.routerId) window.OTP_config.routerId = params.routerId
