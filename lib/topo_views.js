...
var ItineraryTopoView = module.exports.ItineraryTopoView = Backbone.View.extend({
    ...
    _.each(legs.models, function (leg, index) {
      if (leg.isWalk() || leg.isBicycle()) {
        ...
        for (var i = 0; i < latLngs.length - 1; i++) {
          var from = latLngs[i],
            to = latLngs[i + 1]
          legDistDeg += Math.sqrt(Math.pow(to.lat - from.lat, 2) + Math.pow(
              to.lng - from.lng, 2))
        }
        ...
        _.each(leg.get('steps').models, function (step, index) {
          ...
          if (_.isArray(step.get('elevation')))
            elevArray = step.get('elevation'); else {
            var pairs = _([])
        ...
        if (graphArray.length > 0) {
          ...
          var fillStr = pathStr + ' L' + graphArray[graphArray.length - 1]
              [0] + ' ' + graphHeight
...
var LeafletTopoGraphControl = module.exports.LeafletTopoGraphControl = L.Control.extend({
  options: {
    collapsed: true,
