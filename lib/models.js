    ...
    if (!this.attributes.fromPlace) {
      ...
      $.ajax(this.urlRoot, { dataType: 'json', data: utils.filterParams(this.attributes)})
        .done(function(data) {
          ...
          if (data && data.plan && data.plan.from && data.plan.to && data.plan.itineraries && data.plan.itineraries.length > 0) {
            m.trigger('success', m.processRequest(data.plan));
          } else {
            m.trigger('failure', 'Problem finding results for your query. Please enter a valid start and end location.');
          }
        })
