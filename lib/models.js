...
var PlanRequest = module.exports.PlanRequest = Backbone.Model.extend({
  ...
  request: function() {
    if (!this.attributes.fromPlace) {
      this.trigger('failure', 'Please enter a start location');
    } else if (!this.attributes.toPlace) {
      this.trigger('failure', 'Please enter an end location');
    } else {
      log('requesting plan');
      this.trigger('requesting', this);

      var self = this;
      fetch(this.urlRoot + this.toQueryString())
        .then(function(response) {
          log('fetch complete %d', response.status);
        ...
        .then(function(results) {
          ...
          var plan = results && results.plan;
          if (!plan || !plan.itineraries || plan.itineraries.length < 1) {
            return Promise.reject('No results found!');
          } else {
            return Promise.resolve(self.processRequest(plan));
          }
        })
        .then(function(response) {
