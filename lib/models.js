    ...
    if (!this.attributes.fromPlace) {
      ...
      $.ajax(this.urlRoot, { dataType: 'json', data: utils.filterParams(this.attributes)})
        .done(function(data) {
          ...
          if (data.error) {
            m.trigger('failure', 'No transit trips found within 5 miles of your search, try adjusting your start or end locations. Only major metropolitan areas are currently covered. Please check back for expanded data coverage.');
            ...
            if (data.plan.from && data.plan.to) {
              ...
              m.trigger('failure', 'Problem finding results for those locations. Please enter a valid start and end location.');
            }
          } else {
            m.trigger('failure', 'Problem finding results for those locations. Please enter a valid start and end location.');
          }
