 ...
 var EsriGeocoder = {
    ...
    reverse: function(place, error, success) {
 	...
 	lookup : function(query, callback, select) {
        ...
        $.ajax({
            url: OTP.config.esriApi + 'find?text=' + encodeURIComponent(query) + '&outFields=City,Region&bbox=-123.1093389,44.9785603,-122.9184514,44.8745235&f=pjson',
            type: 'GET',
            dataType : "jsonp",
            ...
            success: function(res) {
                    ...
                    for(var itemPos in res.locations.slice(0, 10)) {
                        ...
                        item = {
                            address: res.locations[itemPos].name.split(",")[0],
                            city : res.locations[itemPos].feature.attributes.City,
                            state : res.locations[itemPos].feature.attributes.Region,
                            latlon : res.locations[itemPos].feature.geometry.y + ',' + res.locations[itemPos].feature.geometry.x
                        }
